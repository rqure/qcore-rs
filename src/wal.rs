...
use crate::files::{FileManager, FileManagerTrait, FileConfig, FileInfo};
...
pub struct WalConfig {
    ...
    /// Number of WAL files to create before taking a snapshot
...
/// WAL service for managing write-ahead logs
...
impl WalService {
    pub fn new(config: WalConfig, snapshot_handle: Option<SnapshotHandle>) -> Self {
        Self {
            file_manager: FileManager,
            wal_config: FileConfig {
...
impl WalService {
    ...
    pub fn spawn(config: WalConfig, snapshot_handle: SnapshotHandle, store_handle: StoreHandle) -> WalHandle {
        ...
        tokio::spawn(async move {
            let mut service = WalService::new(config, Some(snapshot_handle));
