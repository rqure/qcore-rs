...
use qlib_rs::{et, ft, Store, Cache, CelExecutor, EntityId, EntitySchema, EntityType, FieldSchema, FieldType, NotificationQueue, NotifyConfig, PageOpts, PageResult, Request, Snapshot, Snowflake, StoreTrait};
...
impl StoreHandle {
    pub fn get_entity_schema(&self, entity_type: &EntityType) -> Result<EntitySchema<qlib_rs::Single>> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
        self.sender.send(StoreRequest::GetEntitySchema {
            entity_type: entity_type.clone(),
            response: response_tx,
        }).map_err(|e| anyhow::anyhow!("Store service has stopped: {}", e))?;
        response_rx.map_err(|e| anyhow::anyhow!("Store service response channel closed: {}", e))?
    }

    pub fn get_complete_entity_schema(&self, entity_type: &EntityType) -> Result<EntitySchema<qlib_rs::Complete>> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
    ...
    pub fn perform_mut(&self, requests: Vec<Request>) -> Result<Vec<Request>> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
    ...
    pub fn perform_map(&self, requests: Vec<Request>) -> Result<HashMap<FieldType, Request>> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
    ...
    pub fn check_requests_authorization(
        &self,
        client_id: &EntityId,
        requests: Vec<Request>,
    ) -> Result<Vec<Request>> {
    ...
    fn handle_request(&mut self, request: StoreRequest) {
        match request {
            StoreRequest::GetEntitySchema { entity_type, response } => {
                let result = self.store.get_entity_schema(&entity_type);
            ...
            StoreRequest::CheckRequestsAuthorization { client_id, requests, response } => {
                let result = self.check_requests_authorization(&client_id, requests);
                if let Err(_) = response.send(result) {
            ...
            StoreRequest::AuthenticateSubject { subject_name, credential, response } => {
                let result = self.authenticate_subject(&subject_name, &credential);
                if let Err(_) = response.send(result) {
            ...
            StoreRequest::SetServices { services, response } => {
                ...
                if !self.write_channel_task_spawned {
                    ...
                    tokio::spawn(async move {
                        Self::process_write_channel_task(write_channel_receiver, config, services);
                    });
...
impl StoreService {
    fn process_write_channel_task(
        ...
        loop {
            ...
            let requests = {
                let mut receiver_guard = write_channel_receiver.lock();
            ...
            Self::process_write_requests(requests, &config, &services);
        }
    }
    ...
    fn process_write_requests(
        ...
        let wal_task = async {
            for request in &requests {
                if let Err(e) = services.wal_handle.write_request(request.clone()) {
                    tracing::error!(
                        error = %e,
        ...
        let sync_task = async {
            if !requests_to_sync.is_empty() {
                ...
                services.peer_handle.send_sync_message(requests_to_sync);
            }
        };
    ...
    fn authenticate_subject(&mut self, subject_name: &str, credential: &str) -> Result<EntityId> {
